...
"""
Example:
    >>> import lib.LoggerPlus import LoggerPlus
    >>> import logging
...
class LoggerPlus:
    ...
    def __init__(self, glbs=None, logdir=None, loglevel=None):
        ...
        """
        if glbs is None:
            logging.error('missing global environment')
            sys.exit(1)
        else:
            self.glbs = glbs

        if logdir is None:
            self.logdir = glbs.get("LOG_DIR", LoggerPlus.DEFAULT_LOGDIR)
        else:
            self.logdir = logdir
        ...
        # add check in envglbs
        self.runid = glbs['runid']
        ...
        ##    self.logfile=self.logDir(self.logdir) + '/' + env.env_dict['pgmname'] + '_' + self.runid + '.log'
    ...
    def setLogger(self, logdir=None, loglevel=None):
        ...
        if loglevel is not None:
            self.loglevel = self.level_str_to_int(loglevel)
            #env.setGlobal('log_level', 'INFO')
        if logdir is not None:
            self.logdir = logdir
            self.logfile = self.logDir(self.logdir) + '/' + self.glbs['pgmname'] + '_' + self.runid + '.log'

        # TODO support update to envProp
...
#        ch_formatter = logging.Formatter('% (name) - 12s: % (levelname) - 8s % (message)s')
        ...
        if self.logfile != None:
            ...
            fh_formatter = logging.Formatter('%(asctime)s %(levelname)s' + ' [' + self.glbs['pgmname'] + '] '
